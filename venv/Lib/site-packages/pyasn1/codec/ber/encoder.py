...
class ChoiceEncoder(AbstractItemEncoder):
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        if asn1Spec is None:
            ...
            if len(names) != 1:
                raise error.PyAsn1Error(
                    '%s components for Choice at %r' % (len(names) and 'Multiple ' or 'None ', value))
    ...
    def __call__(self, value, asn1Spec=None, **options):
        ...
        if LOG:
            LOG('encoder called in %sdef mode, chunk size %s for '
                'type %s, value:\n%s' % (
                not options.get('defMode', True) and 'in' or '', options.get('maxChunkSize', 0),
                asn1Spec is None and value.prettyPrintType() or asn1Spec.prettyPrintType(), value))
