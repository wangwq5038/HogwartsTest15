...
from .. import RESTARTABLE, get_config_parameter, AUTO_BIND_DEFAULT, AUTO_BIND_NONE, AUTO_BIND_NO_TLS, \
...
from ..core.exceptions import LDAPConnectionPoolNameIsMandatoryError, LDAPConnectionPoolNotStartedError, \
    LDAPOperationResult, LDAPExceptionError, LDAPResponseTimeoutError
...
class ReusableStrategy(BaseStrategy):
    ...
    class ConnectionPool(object):
        ...
        def create_pool(self):
            ...
            self.workers = [ReusableStrategy.PooledConnectionWorker(self.master_connection, self.request_queue) for _ in
        ...
        def terminate_pool(self):
            if not self.terminated:
                ...
                for _ in range(len([worker for worker in self.workers if
                                    worker.thread.is_alive()])):  # put a TERMINATE signal on the queue for each active thread
    ...
    class PooledConnectionThread(Thread):
        ...
        def run(self):
            ...
            while not terminate:
                try:
                    counter, message_type, request, controls = pool.request_queue.get(block=True,
                                                                                      timeout=self.master_connection.strategy.pool.keepalive)
                ...
                with self.worker.worker_lock:
                    ...
                    if counter == TERMINATE_REUSABLE:
                        ...
                        if (
                                datetime.now() - self.worker.creation_time).seconds >= self.master_connection.strategy.pool.lifetime:  # destroy and create a new connection
                            try:
                                self.worker.connection.unbind()
                            except LDAPExceptionError:
                                pass
                        ...
                        if message_type not in ['bindRequest', 'unbindRequest']:
                            try:
                                ...
                                if message_type == 'searchRequest':
                                    response = self.worker.connection.post_send_search(
                                        self.worker.connection.send(message_type, request, controls))
                                else:
                                    response = self.worker.connection.post_send_single_response(
                                        self.worker.connection.send(message_type, request, controls))
    ...
    class PooledConnectionWorker(object):
        ...
        def __str__(self):
            ...
            s += ' - time to live: ' + str(
                self.master_connection.strategy.pool.lifetime - (datetime.now() - self.creation_time).seconds)
        ...
        def new_connection(self):
            ...
            self.connection = Connection(
                server=self.master_connection.server_pool if self.master_connection.server_pool else self.master_connection.server,
            ...
            if self.master_connection.auto_bind and self.master_connection.auto_bind not in [AUTO_BIND_NONE,
                                                                                             AUTO_BIND_DEFAULT]:
                if log_enabled(BASIC):
    ...
    def get_response(self, counter, timeout=None, get_request=False):
        ...
        if counter == BOGUS_BIND:  # send a bogus bindResponse
            response = list()
            result = {'description': 'success', 'referrals': None, 'type': 'bindResponse', 'result': 0, 'dn': '',
                      'message': '<bogus Bind response>', 'saslCreds': None}
            ...
            response = list()
            result = {'result': 0, 'referrals': None, 'responseName': '1.3.6.1.4.1.1466.20037', 'type': 'extendedResp',
                      'description': 'success', 'responseValue': 'None', 'dn': '',
                      'message': '<bogus StartTls response>'}
