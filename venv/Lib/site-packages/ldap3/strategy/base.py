...
from ..core.exceptions import LDAPOperationResult, LDAPSASLBindInProgressError, LDAPSocketOpenError, \
...
from ..operation.extended import extended_request_to_dict, extended_response_to_dict, intermediate_response_to_dict, \
    extended_response_to_dict_fast, intermediate_response_to_dict_fast
...
class BaseStrategy(object):
        ...
        if self.connection.lazy and not self.connection._executing_deferred:
            ...
            if self.connection.server_pool:
                new_server = self.connection.server_pool.get_server(
                    self.connection)  # get a server from the server_pool if available
                ...
                if not self.connection.server.current_address and exception_history:
                    if len(exception_history) == 1:  # only one exception, reraise
                        if log_enabled(ERROR):
                            log(ERROR, '<%s> for <%s>',
                                str(exception_history[0][0]) + ' ' + str((exception_history[0][1])), self.connection)
    ...
    def close(self):
        ...
        if self.connection.lazy and not self.connection._executing_deferred and (
                self.connection._deferred_bind or self.connection._deferred_open):
            ...
            if not socket_bind_succeeded:
                ...
                if log_enabled(ERROR):
                    log(ERROR,
                        ...
                        self.connection.source_address, self.connection.source_port_list, self.connection,
                        last_bind_exc)
                raise communication_exception_factory(LDAPSocketOpenError, type(last_bind_exc)(str(last_bind_exc)))(
                    last_bind_exc)
        ...
        if self._outstanding and message_id in self._outstanding:
            ...
            if responses[-2]['result'] == RESULT_REFERRAL:
                ...
                if self.connection.auto_referrals:
                    ref_response, ref_result = self.do_operation_on_referral(self._outstanding[message_id],
            ...
            if self.connection.auto_range and not hasattr(self, '_auto_range_searching') and any(
                    (True for resp in response if 'raw_attributes' in resp for name in resp['raw_attributes'] if
                     ';range=' in name)):
                ...
                for entry in response:
                    if entry['type'] == 'searchResEntry':
                        for attribute_type in self._outstanding[message_id]['attributes']:
                            if attribute_type not in entry['raw_attributes'] and attribute_type not in (
                            ALL_ATTRIBUTES, ALL_OPERATIONAL_ATTRIBUTES, NO_ATTRIBUTES):
    ...
    def decode_response_fast(self, ldap_message):
        ...
        if ldap_message['protocolOp'] == 1:  # bindResponse
            ...
            result = search_result_entry_response_to_dict_fast(ldap_message['payload'], self.connection.server.schema,
                                                               self.connection.server.custom_formatter,
                                                               self.connection.check_names)
    ...
    def do_next_range_search(self, request, response, attr_name):
        ...
        while not done:
            ...
            if high_range != '*':
                if log_enabled(PROTOCOL):
                    log(PROTOCOL, 'performing next search on auto-range <%s> via <%s>', str(int(high_range) + 1),
                        self.connection)
                ...
                if not done:
                    if requested_range in current_response['raw_attributes'] and len(
                            current_response['raw_attributes'][requested_range]) == 0:
    ...
    def create_referral_connection(self, referrals):
        ...
        if valid_referral_list:
            ...
            cachekey = (selected_referral['host'], selected_referral['port'] or self.connection.server.port,
            ...
            if self.connection.use_referral_cache and cachekey in self.referral_cache:
                ...
                referral_connection = Connection(server=referral_server,
                                                 user=self.connection.user if not selected_referral[
                                                     'anonymousBindOnly'] else None,
                                                 password=self.connection.password if not selected_referral[
                                                     'anonymousBindOnly'] else None,
                                                 version=self.connection.version,
                                                 authentication=self.connection.authentication if not selected_referral[
                                                     'anonymousBindOnly'] else ANONYMOUS,
                ...
                if self.connection.tls_started and not referral_server.ssl:  # if the original server was in start_tls mode and the referral server is not in ssl then start_tls on the referral connection
                    if not referral_connection.start_tls():
                        error = 'start_tls in referral not successful' + (
                            ' - ' + referral_connection.last_error if referral_connection.last_error else '')
                        if log_enabled(ERROR):
