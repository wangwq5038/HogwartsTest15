...
class RestartableStrategy(SyncStrategy):
        ...
        if not self._restarting:  # if not already performing a restart
            ...
            while counter > 0:  # includes restartable_tries == True
                if log_enabled(BASIC):
                    log(BASIC, 'try #%d to open Restartable connection <%s>', self.restartable_tries - counter,
                        self.connection)
                ...
                if not self.connection.closed:
                    try:  # resetting connection
                        self.connection.unbind()
                            ...
                            LDAPSocketOpenError):  # don't trace catch socket errors because socket could already be closed
                        pass
                ...
                try:  # reissuing same operation
                    ...
                    SyncStrategy._open_socket(self, address, use_ssl,
                                              unix_socket)  # calls super (not restartable) _open_socket()
                    if self.connection.usage:
    ...
    def send(self, message_type, request, controls=None):
        ...
        if not self._restarting:  # machinery for restartable connection
            ...
            while counter > 0:
                if log_enabled(BASIC):
                    log(BASIC, 'try #%d to send in Restartable connection <%s>', self.restartable_tries - counter,
                        self.connection)
                sleep(self.restartable_sleep_time)
                if not self.connection.closed:
                    try:  # resetting connection
                        self.connection.unbind()
                            ...
                            LDAPSocketOpenError):  # don't trace socket errors because socket could already be closed
                        pass
                    except Exception as e:
                        if log_enabled(ERROR):
                            log(ERROR, '<%s> while restarting <%s>', e, self.connection)
                ...
                try:  # reopening connection
                    ...
                    if self._restart_tls:  # restart tls if start_tls was previously used
                        if self.connection.start_tls(read_server_info=False):
                            error = 'restart tls in restartable not successful' + (
                                ' - ' + self.connection.last_error if self.connection.last_error else '')
                            if log_enabled(ERROR):
                    ...
                    if message_type != 'bindRequest':
                        self.connection.bind(read_server_info=False,
                                             controls=self._last_bind_controls)  # binds with previously used controls unless the request is already a bindRequest
                    if not self.connection.server.schema and not self.connection.server.info:
                        self.connection.refresh_server_info()
                    else:
                        self.connection._fire_deferred(
                            read_info=False)  # in case of lazy connection, not open by the refresh_server_info
                except Exception as e:
                    if log_enabled(ERROR):
                        log(ERROR, '<%s> while restarting <%s>', e, self.connection)
                    self._add_exception_to_history(type(e)(str(e)))
    ...
    def post_send_single_response(self, message_id):
        ...
        try:
            ret_value = SyncStrategy.post_send_single_response(self, self.send(self._current_message_type,
    ...
    def post_send_search(self, message_id):
        ...
        # if an LDAPExceptionError is raised then resend the request
        try:
            ret_value = SyncStrategy.post_send_search(self, self.connection.send(self._current_message_type,
                                                                                 self._current_request,
                                                                                 self._current_controls))
            self._reset_exception_history()
            return ret_value
