...
from ..core.exceptions import LDAPSocketReceiveError, communication_exception_factory, LDAPExceptionError, \
    LDAPExtensionError, LDAPOperationResult
...
class SyncStrategy(BaseStrategy):
        ...
        while receiving:
            if get_more_data:
                try:
                    ...
                    raise communication_exception_factory(LDAPSocketReceiveError, type(e)(str(e)))(
    ...
    def post_send_single_response(self, message_id):
        ...
        if result[
            'type'] == 'intermediateResponse':  # checks that all responses are intermediates (there should be only one)
                ...
                for response in responses:
                    if len(response) > 0:
                        ...
                        if self.connection.fast_decoder:
                            ...
                            ldap_resp, _ = decoder.decode(response,
                                                          asn1Spec=LDAP_MESSAGE_TEMPLATE)  # unprocessed unused because receiving() waits for the whole message
                            dict_response = self.decode_response(ldap_resp)
                        if log_enabled(EXTENDED):
                            log(EXTENDED, 'ldap message received via <%s>:%s', self.connection,
                                format_ldap_message(ldap_resp, '<<'))
                        if int(ldap_resp['messageID']) == message_id:
                            ...
                            if dict_response['type'] not in ['searchResEntry', 'searchResRef', 'intermediateResponse']:
                                response_complete = True
                                     ...
                                     'messageID']) == 0:  # 0 is reserved for 'Unsolicited Notification' from server as per RFC4511 (paragraph 4.4)
                            if dict_response[
                                'responseName'] == '1.3.6.1.4.1.1466.20036':  # Notice of Disconnection as per RFC4511 (paragraph 4.4.1)
                                return SESSION_TERMINATED_BY_SERVER
                            elif dict_response[
                                'responseName'] == '2.16.840.1.113719.1.27.103.4':  # Novell LDAP transaction error unsolicited notification
