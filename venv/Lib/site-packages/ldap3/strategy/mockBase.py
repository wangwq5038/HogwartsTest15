...
class MockBaseStrategy(object):
    ...
    def _prepare_value(self, attribute_type, value, validate=True):
        ...
        if not isinstance(raw_value, bytes):
            raise LDAPInvalidValueError(
                'The value "%s" of type %s for "%s" must be bytes or an offline schema needs to be provided when Mock strategy is used.' % (
                    value,
                    type(value),
                    attribute_type,
                ))
            ...
            if escaped_dn not in self.connection.server.dit:
                ...
                for attribute in attributes:
                    ...
                    if not isinstance(attributes[attribute],
                                      SEQUENCE_TYPES):  # entry attributes are always lists of bytes values
                        attributes[attribute] = [attributes[attribute]]
                    if self.connection.server.schema and self.connection.server.schema.attribute_types[
                        attribute].single_value and len(
                            attributes[attribute]) > 1:  # multiple values in single-valued attribute
                        return False
                            ...
                            while class_set_size != len(class_set):
                                ...
                                for class_name in class_set:
                                    if self.connection.server.schema.object_classes[class_name].superior:
                                        new_classes.update(
                                            self.connection.server.schema.object_classes[class_name].superior)
                                class_set.update(new_classes)
                            new_entry['objectClass'] = [to_raw(value) for value in class_set]
                    else:
                        new_entry[attribute] = [self._prepare_value(attribute, value, validate) for value in
                ...
                for rdn in safe_rdn(escaped_dn, decompose=True):  # adds rdns to entry attributes
                    if rdn[0] not in new_entry:  # if rdn attribute is missing adds attribute and its value
                        ...
                        if raw_rdn not in new_entry[
                            rdn[0]]:  # add rdn value if rdn attribute is present but value is missing
            ...
            for modification in changes:
                ...
                if operation == 0:  # add
                    if attribute not in entry and elements:  # attribute not present, creates the new attribute and add elements
                        if self.connection.server.schema and self.connection.server.schema.attribute_types and \
                                self.connection.server.schema.attribute_types[attribute].single_value and len(
                                elements) > 1:  # multiple values in single-valued attribute
                            result_code = 19
                        ...
                        if self.connection.server.schema and self.connection.server.schema.attribute_types and \
                                self.connection.server.schema.attribute_types[
                                    attribute].single_value:  # multiple values in single-valued attribute
                            result_code = 19
                            message = 'attribute is single-valued'
                        else:
                ...
                if decoded_control[1]['criticality'] and decoded_control[0] not in SEARCH_CONTROLS:
                    ...
                    if not decoded_control[1]['value']['cookie']:  # new paged search
                        ...
                        if result['resultCode'] == RESULT_SUCCESS:  # success
                            paged_set = PagedSearchSet(response, int(decoded_control[1]['value']['size']),
                                                       decoded_control[1]['criticality'])
    ...
    def _execute_search(self, request):
        ...
        filter_root = parse_filter(request['filter'], self.connection.server.schema, auto_escape=True,
                                   auto_encode=False, validator=self.connection.server.custom_validator,
        ...
        if scope == 0:  # base object
            ...
            for entry in self.connection.server.dit:
                if entry.lower().endswith(base.lower()) and ',' not in entry[:-len(
                        base) - 1]:  # only leafs without commas in the remaining dn
                ...
                for match in matched:
                    responses.append({
                        ...
                        'attributes': [{'type': attribute,
                                        'vals': [] if request['typesOnly'] else self.connection.server.dit[match][
                                            attribute]}
    ...
    def evaluate_filter_node(self, node, candidates):
        ...
        if node.tag == ROOT:
                ...
                if attr_name in self.connection.server.dit[candidate]:
                    for value in self.connection.server.dit[candidate][attr_name]:
                        if value.isdigit() and attr_value.isdigit():  # int comparison
                            ...
                            if to_unicode(value, SERVER_ENCODING).lower() >= to_unicode(attr_value,
                                                                                        SERVER_ENCODING).lower():  # case insensitive string comparison
                                node.matched.add(candidate)
                            else:
                                node.unmatched.add(candidate)
                ...
                if attr_name in self.connection.server.dit[candidate]:
                    for value in self.connection.server.dit[candidate][attr_name]:
                        if value.isdigit() and attr_value.isdigit():  # int comparison
                            ...
                            if to_unicode(value, SERVER_ENCODING).lower() <= to_unicode(attr_value,
                                                                                        SERVER_ENCODING).lower():  # case insentive string comparison
                                node.matched.add(candidate)
                            else:
                                node.unmatched.add(candidate)
            ...
            if substring_filter and not node.assertion.get('any', None) and not node.assertion.get('final',
                                                                                                   None):  # only initial, adds .*
                substring_filter += '.*'
    ...
    def _check_equality(value1, value2):
        ...
        if str(value1).isdigit() and str(value2).isdigit():
            if int(value1) == int(value2):  # int comparison
                return True
        try:
            if to_unicode(value1, SERVER_ENCODING).lower() == to_unicode(value2,
                                                                         SERVER_ENCODING).lower():  # case insensitive comparison
                return True
        except UnicodeError:
            pass
