...
class MockAsyncStrategy(MockBaseStrategy,
                        AsyncStrategy):  # class inheritance sequence is important, MockBaseStrategy must be the first one
    """
    ...
    def post_send_search(self, payload):
        ...
        if message_type == 'searchRequest':
            ...
            for entry in responses:
                response = search_result_entry_response_to_dict(entry, self.connection.server.schema,
                ...
                if self.connection.empty_attributes:
                    for attribute_type in request['attributes']:
                        ...
                        if attribute_name not in response['raw_attributes'] and attribute_name not in (
    ...
    def post_send_single_response(self,
                                  payload):  # payload is a tuple sent by self.send() made of message_type, request, controls
        message_id, message_type, request, controls = payload
        responses = []
        result = None
