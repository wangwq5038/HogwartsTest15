...
from .. import DEREF_NEVER, BASE, LEVEL, SUBTREE, DEREF_SEARCH, DEREF_BASE, DEREF_ALWAYS, NO_ATTRIBUTES, SEQUENCE_TYPES, \
    get_config_parameter, STRING_TYPES

from ..core.exceptions import LDAPInvalidFilterError, LDAPAttributeError, LDAPInvalidScopeError, \
...
from ..protocol.convert import ava_to_dict, attributes_to_list, search_refs_to_list, validate_assertion_value, \
    prepare_filter_for_sending, search_refs_to_list_fast
...
class FilterNode(object):
    ...
    def __repr__(self, pos=0):
        node_tags = ['ROOT', 'AND', 'OR', 'NOT', 'MATCH_APPROX', 'MATCH_GREATER_OR_EQUAL', 'MATCH_LESS_OR_EQUAL',
...
def evaluate_match(match, schema, auto_escape, auto_encode, validator, check_names):
    ...
    if left_part.endswith('~'):  # approximate match '~='
        ...
        assertion = {'attr': left_part,
                     'value': validate_assertion_value(schema, left_part, right_part, auto_escape, auto_encode,
                                                       validator, check_names)}
        ...
        assertion = {'attr': attribute_name,
                     'value': validate_assertion_value(schema, attribute_name, right_part, auto_escape, auto_encode,
                                                       validator, check_names), 'matchingRule': matching_rule,
        ...
        initial = validate_assertion_value(schema, left_part, substrings[0], auto_escape, auto_encode, validator,
                                           check_names) if substrings[0] else None
        final = validate_assertion_value(schema, left_part, substrings[-1], auto_escape, auto_encode, validator,
                                         check_names) if substrings[-1] else None
        any_string = [
            validate_assertion_value(schema, left_part, substring, auto_escape, auto_encode, validator, check_names) for
            substring in substrings[1:-1] if substring]
...
def parse_filter(search_filter, schema, auto_escape, auto_encode, validator, check_names):
    ...
    if search_filter and search_filter.count('(') == search_filter.count(')') and search_filter.startswith(
            '(') and search_filter.endswith(')'):
            ...
            if skip_white_space and c in whitespace:
                ...
                if just_closed:
                    ...
                    if start_pos:
                        ...
                        current_node.append(
                            evaluate_match(search_filter[start_pos:end_pos], schema, auto_escape, auto_encode,
                                           validator, check_names))
                start_pos = None
                state = SEARCH_OPEN_OR_CLOSE
...
def compile_filter(filter_node):
    ...
    if filter_node.tag == AND:
        ...
        compiled_filter.setComponentByName('notFilter', boolean_filter,
                                           verifyConstraints=False)  # do not verify constraints because of hack for recursive filters in rfc4511
        ...
        if 'initial' in filter_node.assertion and filter_node.assertion['initial']:
            substrings[pos] = Substring().setComponentByName('initial', Initial(
                prepare_filter_for_sending(filter_node.assertion['initial'])))
            pos += 1
        ...
        if 'final' in filter_node.assertion and filter_node.assertion['final']:
            substrings[pos] = Substring().setComponentByName('final', Final(
                prepare_filter_for_sending(filter_node.assertion['final'])))
    ...
    for index, attribute in enumerate(attribute_list):
        if schema and schema.attribute_types:
            if ';' in attribute:  # exclude tags from validation
                if not attribute[0:attribute.index(
                        ';')] in schema.attribute_types and attribute.lower() not in conf_attributes_excluded_from_check:
...
def search_operation(search_base,
    ...
    request['filter'] = compile_filter(
        parse_filter(search_filter, schema, auto_escape, auto_encode, validator, check_names).elements[
            0])  # parse the searchFilter string and compile it starting from the root node
...
def checked_attributes_to_dict(attribute_list, schema=None, custom_formatter=None):
    ...
    for attribute in attribute_list:
        ...
        checked_attributes[name] = format_attribute_values(schema, name, decode_raw_vals(attribute['vals']) or [],
                                                           custom_formatter)
...
def checked_attributes_to_dict_fast(attribute_list, schema=None, custom_formatter=None):
    ...
    for attribute in attribute_list:
        ...
        checked_attributes[name] = format_attribute_values(schema, name, decode_raw_vals_fast(attribute[3][1][3]) or [],
                                                           custom_formatter)
    return checked_attributes
        ...
        for substring in filter_object['substringFilter']['substrings']:
            ...
            if substring[component] is not None and substring[component].hasValue():
                if component == 'initial':
                    ...
                    filter_string += str(substring['any']) if filter_string.endswith('*') else '*' + str(
                        substring['any'])
                    filter_string += '*'
...
def search_result_entry_response_to_dict_fast(response, schema, custom_formatter, check_names):
    ...
    if check_names:
        entry_dict['attributes'] = checked_attributes_to_dict_fast(response[1][3], schema,
                                                                   custom_formatter)  # attributes
    else:
