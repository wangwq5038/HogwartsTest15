...
def extended_operation(request_name,
    ...
    elif str is not bytes and isinstance(request_value,
                                         (bytes, bytearray)):  # in Python 3 doesn't try to encode a byte value
...
def extended_request_to_dict(request):
    ...
    return {'name': str(request['requestName']),
            'value': bytes(request['requestValue']) if 'requestValue' in request and request[
                'requestValue'] is not None and request['requestValue'].hasValue() else None}
...
def extended_response_to_dict(response):
    return {'result': int(response['resultCode']),
            ...
            'responseName': str(response['responseName']) if response['responseName'] is not None and response[
                'responseName'].hasValue() else str(),
            'responseValue': bytes(response['responseValue']) if response['responseValue'] is not None and response[
                'responseValue'].hasValue() else bytes()}
