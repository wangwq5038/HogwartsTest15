...
from ..core.exceptions import LDAPUserNameIsMandatoryError, LDAPPasswordIsMandatoryError, \
    LDAPUnknownAuthenticationMethodError, LDAPUserNameNotAllowedError
    ...
    if authentication == SIMPLE:
        ...
        if password:
            request['authentication'] = AuthenticationChoice().setComponentByName('simple', Simple(
                validate_simple_password(password)))
        else:
        ...
        request['name'] = ''
        request['authentication'] = AuthenticationChoice().setComponentByName('sicilyPackageDiscovery',
                                                                              SicilyPackageDiscovery(''))
    elif authentication == 'SICILY_NEGOTIATE_NTLM':  # https://msdn.microsoft.com/en-us/library/cc223501.aspx
        request['name'] = 'NTLM'
        request['authentication'] = AuthenticationChoice().setComponentByName('sicilyNegotiate', SicilyNegotiate(
            name.create_negotiate_message()))  # ntlm client in self.name
        ...
        if server_creds:
            request['name'] = ''
            request['authentication'] = AuthenticationChoice().setComponentByName('sicilyResponse',
                                                                                  SicilyResponse(server_creds))
        else:
            request = None
    else:
...
def bind_response_to_dict(response):
    return {'result': int(response['resultCode']),
            ...
            'referrals': referrals_to_list(response['referral']) if response['referral'] is not None and response[
                'referral'].hasValue() else [],
            'saslCreds': bytes(response['serverSaslCreds']) if response['serverSaslCreds'] is not None and response[
                'serverSaslCreds'].hasValue() else None}
