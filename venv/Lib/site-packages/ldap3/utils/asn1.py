...
def decode_message_fast(message):
    ber_len, ber_value_offset = compute_ber_size(
        get_bytes(message[:10]))  # get start of sequence, at maximum 3 bytes for length
...
def decode_sequence(message, start, stop, context_decoders=None):
    ...
    while start < stop:
        ...
        if ber_decoder:
            ...
            # try:
            value = context_decoders[ber_type](message, start,
                                               start + ber_len)  # call value decode function for context class
            # except KeyError:
...
def ldap_result_to_dict_fast(response):
    ...
    if len(response) == 4:
        response_dict['referrals'] = referrals_to_list(
            [to_unicode(referral[3], from_server=True) for referral in response[3][3]])  # referrals
    else:
