    ...
    while equal > 0:  # not first character
        if ava[equal - 1] != '\\':  # not an escaped equal so it must be an ava separator
            ...
            if strip:
                ...
                attribute_value = _escape_attribute_value(ava[equal + 1:].strip()) if escape else ava[
                                                                                                  equal + 1:].strip()
            else:
...
def _validate_attribute_type(attribute_type):
    ...
    for c in attribute_type:
        if not (
                c in ascii_letters or c in digits or c == '-'):  # allowed uppercase and lowercase letters, digits and hyphen as per RFC 4512
    ...
    if attribute_type[0] in digits or attribute_type[0] == '-':  # digits and hyphen not allowed as first character
        raise LDAPInvalidDnError(
            'character \'' + attribute_type[0] + '\' not allowed as first character of attribute type')
...
def parse_dn(dn, escape=False, strip=False):
    ...
    avas = []
    while dn:
        ava, separator = _get_next_ava(
            dn)  # if returned ava doesn't containg any unescaped equal it'a appended to last ava in avas
...
def safe_dn(dn, decompose=False, reverse=False):
    ...
    if dn.startswith('<GUID=') and dn.endswith(
            '>'):  # Active Directory allows looking up objects by putting its GUID in a specially-formatted DN (e.g. '<GUID=7b95f0d5-a3ed-486c-919c-077b8c9731f2>')
        escaped_dn = dn
    elif dn.startswith('<WKGUID=') and dn.endswith(
            '>'):  # Active Directory allows Binding to Well-Known Objects Using WKGUID in a specially-formatted DN (e.g. <WKGUID=a9d1ca15768811d1aded00c04fd8d5cd,dc=Fabrikam,dc=com>)
        escaped_dn = dn
    elif dn.startswith('<SID=') and dn.endswith(
            '>'):  # Active Directory allows looking up objects by putting its security identifier (SID) in a specially-formatted DN (e.g. '<SID=S-#-#-##-##########-##########-##########-######>')
        escaped_dn = dn
