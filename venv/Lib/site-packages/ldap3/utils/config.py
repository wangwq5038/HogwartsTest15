...
_ADDITIONAL_SERVER_ENCODINGS = ['latin-1',
                                'koi8-r']  # some broken LDAP implementation may have different encoding than those expected by RFCs
...
def get_config_parameter(parameter):
    if parameter == 'CASE_INSENSITIVE_ATTRIBUTE_NAMES':  # Boolean
    ...
    elif parameter in ['DEFAULT_CLIENT_ENCODING',
                       'DEFAULT_ENCODING']:  # String - DEFAULT_ENCODING for backward compatibility
        ...
        if isinstance(_UTF8_ENCODED_TYPES, SEQUENCE_TYPES):
            ...
            return [_UTF8_ENCODED_TYPES]
    elif parameter in ['ADDITIONAL_SERVER_ENCODINGS',
                       'ADDITIONAL_ENCODINGS']:  # Sequence - ADDITIONAL_ENCODINGS for backward compatibility
