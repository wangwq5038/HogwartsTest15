    ...
    if not error:
        for member in members_dn:
            if fix:  # checks for existance of member and for already assigned groups
                result = connection.search(member, '(objectclass=*)', BASE, dereference_aliases=DEREF_NEVER,
                ...
                existing_security_equals = response[0]['attributes']['securityEquals'] if 'securityEquals' in \
                                                                                          response[0][
                                                                                              'attributes'] else []
                existing_group_membership = response[0]['attributes']['groupMembership'] if 'groupMembership' in \
                                                                                            response[0][
                                                                                                'attributes'] else []
            ...
            changes = dict()
            security_equals_to_add = [element for element in groups_dn if
                                      element.lower() not in existing_security_equals]
            group_membership_to_add = [element for element in groups_dn if
                                       element.lower() not in existing_group_membership]
    ...
    if not error:
        for group in groups_dn:
            if fix:  # checks for existance of group and for already assigned members
                ...
                existing_equivalent_to_me = response[0]['attributes']['equivalentToMe'] if 'equivalentToMe' in \
                                                                                           response[0][
                                                                                               'attributes'] else []
            ...
            equivalent_to_me_to_add = [element for element in members_dn if
                                       element.lower() not in existing_equivalent_to_me]
            if member_to_add:
