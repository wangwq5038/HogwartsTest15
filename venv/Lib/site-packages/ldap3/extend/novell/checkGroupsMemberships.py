...
def _check_members_have_memberships(connection,
    ...
    for member in members_dn:
        result = connection.search(member, '(objectclass=*)', BASE, dereference_aliases=DEREF_NEVER,
                                   attributes=['groupMembership', 'securityEquals'])
        ...
        existing_security_equals = response[0]['attributes']['securityEquals'] if 'securityEquals' in response[0][
            'attributes'] else []
        existing_group_membership = response[0]['attributes']['groupMembership'] if 'groupMembership' in response[0][
            'attributes'] else []
...
def _check_groups_contain_members(connection,
    ...
    for group in groups_dn:
        ...
        existing_equivalent_to_me = response[0]['attributes']['equivalentToMe'] if 'equivalentToMe' in response[0][
            'attributes'] else []
...
def edir_check_groups_memberships(connection,
    ...
    try:
        members_have_memberships, partial_member_security = _check_members_have_memberships(connection, members_dn,
                                                                                            groups_dn)
        groups_contain_members, partial_group_security = _check_groups_contain_members(connection, groups_dn,
                                                                                       members_dn)
