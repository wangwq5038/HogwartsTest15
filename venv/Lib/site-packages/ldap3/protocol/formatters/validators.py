...
def validate_zero_and_minus_one_and_positive_int(input_value):
    ...
    if not isinstance(input_value, SEQUENCE_TYPES):
        ...
        if len(input_value) == 1 and (
                isinstance(input_value[0], NUMERIC_TYPES) or isinstance(input_value[0], STRING_TYPES)):
...
def validate_time_with_0_year(input_value):
    ...
    for element in input_value:
        ...
        if isinstance(element, STRING_TYPES):  # tries to check if it is already be a Generalized Time
            if element.startswith('0000') or isinstance(format_time(to_raw(element)),
                                                        datetime):  # valid Generalized Time string
    ...
    for element in input_value:
        ...
        if isinstance(element, NUMERIC_TYPES):
            ...
            if element.tzinfo:  # a datetime with a timezone
                ...
                offset = datetime.now() - datetime.utcnow()
                valid_values.append(
                    to_raw((timegm((element - offset).timetuple()) + 11644473600) * 10000000, encoding='ascii'))
        else:
            return False
            ...
            if element[0] == '{' and element[-1] == '}':
                try:
                    valid_values.append(UUID(
                        hex=element).bytes_le)  # string representation, value in big endian, converts to little endian
                    changed = True
                except ValueError:
                    error = True
            elif '-' in element:
                ...
                try:
                    valid_values.append(UUID(
                        bytes_le=ldap_escape_to_bytes(element)).bytes_le)  # byte representation, value in little endian
                    changed = True
                except ValueError:
                    error = True
            elif '-' not in element:  # value in little endian
                try:
                    valid_values.append(UUID(bytes_le=a2b_hex(
                        element)).bytes_le)  # packet representation, value in little endian, converts to little endian
                    changed = True
                except ValueError:
                    error = True
