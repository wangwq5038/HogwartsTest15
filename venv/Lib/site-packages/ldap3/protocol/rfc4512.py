...
class BaseServerInfo(object):
    ...
    def from_json(cls, json_definition, schema=None, custom_formatter=None):
        ...
        if schema:
            for attribute in definition['raw']:
                ...
                attributes[attribute] = format_attribute_values(schema, attribute,
                                                                [value for value in definition['raw'][attribute]],
                                                                custom_formatter)
        else:
            for attribute in definition['raw']:
    ...
    def to_json(self,
        ...
        return json.dumps(json_dict, ensure_ascii=False, sort_keys=sort, indent=indent, check_circular=True,
...
class DsaInfo(BaseServerInfo):
    ...
    def __init__(self, attributes, raw_attributes):
        ...
        self.supported_features = decode_oids(attributes.pop('supportedFeatures', None)) + decode_oids(
    ...
    def __repr__(self):
        ...
        if self.supported_ldap_versions:
            if isinstance(self.supported_ldap_versions, SEQUENCE_TYPES):
                r += ('  Supported LDAP versions: ' + ', '.join(
                    [str(s) for s in self.supported_ldap_versions])) if self.supported_ldap_versions else ''
            else:
        ...
        if self.naming_contexts:
            if isinstance(self.naming_contexts, SEQUENCE_TYPES):
                r += ('  Naming contexts: ' + linesep + linesep.join(
                    ['    ' + str(s) for s in self.naming_contexts])) if self.naming_contexts else ''
            else:
        ...
        if self.alt_servers:
            if isinstance(self.alt_servers, SEQUENCE_TYPES):
                r += ('  Alternative servers: ' + linesep + linesep.join(
                    ['    ' + str(s) for s in self.alt_servers])) if self.alt_servers else ''
            else:
        ...
        if self.supported_controls:
            if isinstance(self.supported_controls, SEQUENCE_TYPES):
                r += ('  Supported controls: ' + linesep + linesep.join(
                    ['    ' + oid_to_string(s) for s in self.supported_controls])) if self.supported_controls else ''
            else:
                r += ('  Supported controls: ' + str(self.supported_controls))
            r += linesep
        if self.supported_extensions:
            if isinstance(self.supported_extensions, SEQUENCE_TYPES):
                r += ('  Supported extensions: ' + linesep + linesep.join(['    ' + oid_to_string(s) for s in
                                                                           self.supported_extensions])) if self.supported_extensions else ''
            else:
        ...
        if self.supported_features:
            if self.supported_features:
                if isinstance(self.supported_features, SEQUENCE_TYPES):
                    r += ('  Supported features: ' + linesep + linesep.join(['    ' + oid_to_string(s) for s in
                                                                             self.supported_features])) if self.supported_features else ''
                else:
        ...
        if self.supported_sasl_mechanisms:
            if isinstance(self.supported_sasl_mechanisms, SEQUENCE_TYPES):
                r += ('  Supported SASL mechanisms: ' + linesep + '    ' + ', '.join(
                    [str(s) for s in self.supported_sasl_mechanisms])) if self.supported_sasl_mechanisms else ''
            else:
        ...
        if self.schema_entry:
            if isinstance(self.schema_entry, SEQUENCE_TYPES):
                r += ('  Schema entry: ' + linesep + linesep.join(
                    ['    ' + str(s) for s in self.schema_entry])) if self.schema_entry else ''
            else:
        ...
        for k, v in self.other.items():
            ...
            try:
                ...
                r += (linesep.join(['    ' + str(escape_bytes(s)) for s in v])) if isinstance(v,
                                                                                              SEQUENCE_TYPES) else str(
                    escape_bytes(v))
            r += linesep
        return r
    ...
    def __repr__(self):
        ...
        if isinstance(self.attribute_types, SEQUENCE_TYPES):
            r += ('  Attribute types:' + linesep + '    ' + ', '.join(
                [str(self.attribute_types[s]) for s in self.attribute_types])) if self.attribute_types else ''
        else:
            r += ('  Attribute types:' + str(self.attribute_types))
        r += linesep
        if isinstance(self.object_classes, SEQUENCE_TYPES):
            r += ('  Object classes:' + linesep + '    ' + ', '.join(
                [str(self.object_classes[s]) for s in self.object_classes])) if self.object_classes else ''
        else:
            r += ('  Object classes:' + str(self.object_classes))
        r += linesep
        if isinstance(self.matching_rules, SEQUENCE_TYPES):
            r += ('  Matching rules:' + linesep + '    ' + ', '.join(
                [str(self.matching_rules[s]) for s in self.matching_rules])) if self.matching_rules else ''
        else:
        ...
        if isinstance(self.matching_rule_uses, SEQUENCE_TYPES):
            r += ('  Matching rule uses:' + linesep + '    ' + ', '.join(
                [str(self.matching_rule_uses[s]) for s in self.matching_rule_uses])) if self.matching_rule_uses else ''
        else:
            r += ('  Matching rule uses:' + str(self.matching_rule_uses))
        r += linesep
        if isinstance(self.dit_content_rules, SEQUENCE_TYPES):
            r += ('  DIT content rules:' + linesep + '    ' + ', '.join(
                [str(self.dit_content_rules[s]) for s in self.dit_content_rules])) if self.dit_content_rules else ''
        else:
            r += ('  DIT content rules:' + str(self.dit_content_rules))
        r += linesep
        if isinstance(self.dit_structure_rules, SEQUENCE_TYPES):
            r += ('  DIT structure rules:' + linesep + '    ' + ', '.join([str(self.dit_structure_rules[s]) for s in
                                                                           self.dit_structure_rules])) if self.dit_structure_rules else ''
        else:
            r += ('  DIT structure rules:' + str(self.dit_structure_rules))
        r += linesep
        if isinstance(self.name_forms, SEQUENCE_TYPES):
            r += ('  Name forms:' + linesep + '    ' + ', '.join(
                [str(self.name_forms[s]) for s in self.name_forms])) if self.name_forms else ''
        else:
            r += ('  Name forms:' + str(self.name_forms))
        r += linesep
        if isinstance(self.ldap_syntaxes, SEQUENCE_TYPES):
            r += ('  LDAP syntaxes:' + linesep + '    ' + ', '.join(
                [str(self.ldap_syntaxes[s]) for s in self.ldap_syntaxes])) if self.ldap_syntaxes else ''
        else:
...
class BaseObjectInfo(object):
    ...
    def __repr__(self):
        ...
        r += (linesep + '  Extensions:' + linesep + linesep.join(
            ['    ' + s[0] + ': ' + list_to_string(s[1]) for s in self.extensions])) if self.extensions else ''
        r += (linesep + '  Experimental:' + linesep + linesep.join(
            ['    ' + s[0] + ': ' + list_to_string(s[1]) for s in self.experimental])) if self.experimental else ''
            ...
            if object_definition[0] == '(' and object_definition[-1] == ')':
                    ...
                    if key == 'OID':
                        ...
                        if not conf_ignore_malformed_schema:
                            raise LDAPSchemaError(
                                'malformed schema definition key:' + key + ' - use get_info=NONE in Server definition')
                        else:
                ...
                if hasattr(object_def, 'name') and object_def.name:
                    if conf_case_insensitive_schema:
                        ...
                        ret_dict.set_alias(object_def.name[0], object_def.name[1:] + [object_def.oid],
...
class AttributeTypeInfo(BaseObjectInfo):
    ...
    def __repr__(self):
        ...
        r += (linesep + '  Syntax: ' + (
                    self.syntax + (' [' + str(decode_syntax(self.syntax)))) + ']') if self.syntax else ''
...
class DitContentRuleInfo(BaseObjectInfo):
    ...
    def __repr__(self):
        r = (linesep + '  Auxiliary classes: ' + list_to_string(
            self.auxiliary_classes)) if self.auxiliary_classes else ''
