    ...
    if 'entry' in entry:
        ...
        if 'changes' in entry:
            for change in entry['changes']:
                lines.append(
                    ['add', 'delete', 'replace', 'increment'][change['operation']] + ': ' + change['attribute']['type'])
...
def modify_dn_request_to_ldif(entry, all_base64, sort_order=None):
    ...
    if 'entry' in entry:
        ...
        if control_lines:
            lines.extend(control_lines)
        lines.append('changetype: modrdn') if 'newSuperior' in entry and entry['newSuperior'] else lines.append(
            'changetype: moddn')
...
def operation_to_ldif(operation_type, entries, all_base64=False, sort_order=None):
    ...
    for line in lines:
        if line:
            ...
            ldif_record.extend([' ' + line[i: i + conf_ldif_line_length - 1] for i in
                                range(conf_ldif_line_length, len(line), conf_ldif_line_length - 1)] if len(
                line) > conf_ldif_line_length else [])
        else:
            ldif_record.append('')
...
def decode_persistent_search_control(change):
    if 'controls' in change and '2.16.840.1.113730.3.4.7' in change['controls']:
        decoded = dict()
        decoded_control, unprocessed = decoder.decode(change['controls']['2.16.840.1.113730.3.4.7']['value'],
                                                      asn1Spec=EntryChangeNotificationControl())
        if unprocessed:
        ...
        decoded['changeNumber'] = decoded_control['changeNumber'] if 'changeNumber' in decoded_control and \
                                                                     decoded_control['changeNumber'] is not None and \
                                                                     decoded_control[
                                                                         'changeNumber'].hasValue() else None
        decoded['previousDN'] = decoded_control['previousDN'] if 'previousDN' in decoded_control and decoded_control[
            'previousDN'] is not None and decoded_control['previousDN'].hasValue() else None
        return decoded
