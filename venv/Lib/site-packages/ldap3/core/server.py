...
from .. import DSA, SCHEMA, ALL, BASE, get_config_parameter, OFFLINE_EDIR_8_8_8, OFFLINE_EDIR_9_1_4, OFFLINE_AD_2012_R2, \
    OFFLINE_SLAPD_2_4, OFFLINE_DS389_1_3_3, SEQUENCE_TYPES, IP_SYSTEM_DEFAULT, IP_V4_ONLY, IP_V6_ONLY, IP_V4_PREFERRED, \
    IP_V6_PREFERRED, STRING_TYPES
from .exceptions import LDAPInvalidServerError, LDAPDefinitionError, LDAPInvalidPortError, \
    LDAPInvalidTlsSpecificationError, LDAPSocketOpenError, LDAPInfoError
...
class Server(object):
    ...
    def __repr__(self):
        ...
        r += '' if not self.allowed_referral_hosts else ', allowed_referral_hosts={0.allowed_referral_hosts!r}'.format(
            self)
    ...
    def address_info(self):
        ...
        if not self._address_info or (
                datetime.now() - self._address_info_resolved_time).seconds > conf_refresh_interval:
                ...
                if self.ipc:
                    ...
                    if self.mode == IP_V4_ONLY:
                        addresses = socket.getaddrinfo(self.host, self.port, socket.AF_INET, socket.SOCK_STREAM,
                                                       socket.IPPROTO_TCP, socket.AI_ADDRCONFIG | socket.AI_V4MAPPED)
                    elif self.mode == IP_V6_ONLY:
                        addresses = socket.getaddrinfo(self.host, self.port, socket.AF_INET6, socket.SOCK_STREAM,
                                                       socket.IPPROTO_TCP, socket.AI_ADDRCONFIG | socket.AI_V4MAPPED)
                    else:
        ...
        for address in self.candidate_addresses():
            ...
            try:
                ...
                if not socket_bind_succeeded:
                    if log_enabled(ERROR):
                        log(ERROR,
                            'Unable to locally bind to local address <%s> with any of the source ports <%s> due to <%s>',
                            bind_address, candidate_bind_ports, last_bind_exc)
                    raise LDAPSocketOpenError(
                        'Unable to bind socket locally to address {} with any of the source ports {} due to {}'
                        .format(bind_address, candidate_bind_ports, last_bind_exc))

                if self.connect_timeout:
                    temp_socket.settimeout(self.connect_timeout)
                else:
                    temp_socket.settimeout(
                        conf_availability_timeout)  # set timeout for checking availability to default
                try:
        ...
        if not connection.strategy.pooled:  # in pooled strategies get_dsa_info is performed by the worker threads
            ...
            with self.dit_lock:
                if connection.strategy.sync:  # sync request
                    self._dsa_info = DsaInfo(response[0]['attributes'],
                                             response[0]['raw_attributes']) if status else self._dsa_info
    ...
    def _get_schema_info(self, connection, entry=''):
        ...
        if self._dsa_info and entry == '':  # subschemaSubentry already present in dsaInfo
            ...
            result = connection.search(entry, '(objectClass=*)', BASE, attributes=['subschemaSubentry'],
                                       get_operational_attributes=True)
            if connection.strategy.thread_safe:
                status, result, response, _ = result
            else:
            ...
            if connection.strategy.sync:  # sync request
                ...
                results, _ = connection.get_response(status)
                if len(results) == 1 and 'raw_attributes' in results[0] and 'subschemaSubentry' in results[0][
                    'attributes']:
        ...
        if schema_entry and not connection.strategy.pooled:  # in pooled strategies get_schema_info is performed by the worker threads
            ...
            with self.dit_lock:
                ...
                if status:
                    if connection.strategy.sync:  # sync request
                        self._schema_info = SchemaInfo(schema_entry, response[0]['attributes'],
                                                       response[0]['raw_attributes'])
                    ...
                    if self._schema_info:  # if schema is valid tries to apply formatter to the "other" dict with raw values for schema and info
                        for attribute in self._schema_info.other:
                            self._schema_info.other[attribute] = format_attribute_values(self._schema_info, attribute,
                                                                                         self._schema_info.raw[
                                                                                             attribute],
                        ...
                        if self._dsa_info:  # try to apply formatter to the "other" dict with dsa info raw values
                            for attribute in self._dsa_info.other:
                                self._dsa_info.other[attribute] = format_attribute_values(self._schema_info, attribute,
                                                                                          self._dsa_info.raw[attribute],
                                                                                          self.custom_formatter)
            if log_enabled(BASIC):
    ...
    def from_definition(host, dsa_info, dsa_schema, port=None, use_ssl=False, formatter=None, validator=None):
        ...
        if isinstance(host, SEQUENCE_TYPES):
            dummy = Server(host=host[0], port=port, use_ssl=use_ssl, formatter=formatter, validator=validator,
                           get_info=ALL)  # for ServerPool object
        else:
            dummy = Server(host=host, port=port, use_ssl=use_ssl, formatter=formatter, validator=validator,
                           get_info=ALL)
        if isinstance(dsa_info, DsaInfo):
        ...
        if self.ipc:
            ...
            if addresses:
                if self.mode == IP_SYSTEM_DEFAULT:
                    ...
                    candidates = [address for address in addresses if
                                  address[0] == socket.AF_INET and (address[5] or address[5] is None)]
                elif self.mode == IP_V6_ONLY:
                    candidates = [address for address in addresses if
                                  address[0] == socket.AF_INET6 and (address[5] or address[5] is None)]
            ...
            if log_enabled(BASIC):
                for candidate in candidates:
                    log(BASIC, 'obtained candidate address for <%s>: <%r> with mode %s', self, candidate[:-2],
                        self.mode)
        return candidates
