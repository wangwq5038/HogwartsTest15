...
from .exceptions import LDAPSSLNotSupportedError, LDAPSSLConfigurationError, LDAPCertificateError, \
    start_tls_exception_factory, LDAPStartTLSError
...
class Tls(object):
    ...
    def wrap_socket(self, connection, do_handshake=False):
        ...
        if use_ssl_context:
            ...
            if self.certificate_file:
                ssl_context.load_cert_chain(self.certificate_file, keyfile=self.private_key_file,
            ...
            if self.sni:
                wrapped_socket = ssl_context.wrap_socket(connection.socket, server_side=False,
                                                         do_handshake_on_connect=do_handshake, server_hostname=self.sni)
            else:
                wrapped_socket = ssl_context.wrap_socket(connection.socket, server_side=False,
    ...
    def start_tls(self, connection):
        ...
        if (
                connection.tls_started and not connection._executing_deferred) or connection.strategy._outstanding or connection.sasl_in_progress:
            # Per RFC 4513 (3.1.1)
            if log_enabled(ERROR):
...
def check_hostname(sock, server_name, additional_names):
    ...
    if additional_names:
        host_names = [server_name] + (
            additional_names if isinstance(additional_names, SEQUENCE_TYPES) else [additional_names])
    else:
        host_names = [server_name]
    ...
    for host_name in host_names:
        ...
        try:
            match_hostname(server_certificate,
                           host_name)  # raise CertificateError if certificate doesn't match server name
            if log_enabled(NETWORK):
