...
from .. import ANONYMOUS, SIMPLE, SASL, MODIFY_ADD, MODIFY_DELETE, MODIFY_REPLACE, get_config_parameter, DEREF_ALWAYS, \
    SUBTREE, ASYNC, SYNC, NO_ATTRIBUTES, ALL_ATTRIBUTES, ALL_OPERATIONAL_ATTRIBUTES, MODIFY_INCREMENT, LDIF, \
    ASYNC_STREAM, \
...
from .exceptions import LDAPUnknownStrategyError, LDAPBindError, LDAPUnknownAuthenticationMethodError, \
    LDAPSASLMechanismNotSupportedError, LDAPObjectClassError, LDAPConnectionIsReadOnlyError, LDAPChangeError, \
    LDAPExceptionError, \
    LDAPObjectError, LDAPSocketReceiveError, LDAPAttributeError, LDAPInvalidValueError, LDAPInvalidPortError, \
    LDAPStartTLSError
...
class Connection(object):
        ...
        if self.auto_bind and self.auto_bind not in [AUTO_BIND_NONE, AUTO_BIND_DEFAULT]:
            ...
            if self.auto_bind == AUTO_BIND_NO_TLS:
                ...
                if self.start_tls(read_server_info=False):
                    ...
                    error = 'automatic start_tls befored bind not successful' + (
                ...
                if not self.start_tls(read_server_info=True):
                    error = 'automatic start_tls after bind not successful' + (
                        ' - ' + self.last_error if self.last_error else '')
                    if log_enabled(ERROR):
                        log(ERROR, '%s for <%s>', error, self)
                    self.unbind()
    ...
    def __str__(self):
        s = [
            ...
            'tls not started' if not self.tls_started else (
                'deferred start_tls' if self._deferred_start_tls else 'tls started'),
    ...
    def __repr__(self):
        ...
        r += '' if self.empty_attributes is None else (
                    ', return_empty_attributes=' + ('True' if self.empty_attributes else 'False'))
        ...
        r += '' if self.use_referral_cache is None else (
                    ', use_referral_cache=' + ('True' if self.use_referral_cache else 'False'))
        r += ')'
    ...
    def repr_with_sensitive_data_stripped(self):
        ...
        if self.server_pool:
            ...
            r = 'Connection(server={0.server!r}'.format(self)
        r += '' if self.user is None else ', user={0.user!r}'.format(self)
        r += '' if self.password is None else ", password='{0}'".format(
            '<stripped %d characters of sensitive data>' % len(self.password))
        r += '' if self.auto_bind is None else ', auto_bind={0.auto_bind!r}'.format(self)
        r += '' if self.version is None else ', version={0.version!r}'.format(self)
        r += '' if self.authentication is None else ', authentication={0.authentication!r}'.format(self)
        r += '' if self.strategy_type is None else ', client_strategy={0.strategy_type!r}'.format(self)
        ...
        if self.sasl_mechanism == DIGEST_MD5:
            r += '' if self.sasl_credentials is None else ", sasl_credentials=({0!r}, {1!r}, '{2}', {3!r})".format(
                self.sasl_credentials[0], self.sasl_credentials[1], '*' * len(self.sasl_credentials[2]),
                self.sasl_credentials[3])
        else:
            r += '' if self.sasl_credentials is None else ', sasl_credentials={0.sasl_credentials!r}'.format(self)
        r += '' if self.check_names is None else ', check_names={0.check_names!r}'.format(self)
        r += '' if self.usage is None else (', collect_usage=' + 'True' if self.usage else 'False')
            ...
            if self.lazy and not self._executing_deferred:
                ...
                if self.authentication == ANONYMOUS:
                    ...
                    if not self.strategy.pooled:
                        ...
                        if log_enabled(PROTOCOL):
                            log(PROTOCOL, 'anonymous BIND request <%s> sent via <%s>', bind_request_to_dict(request),
                                self)
                ...
                if not self.strategy.sync and not self.strategy.pooled and self.authentication not in (SASL,
                                                                                                       NTLM):  # get response if asynchronous except for SASL and NTLM that return the bind result even for asynchronous strategy
                ...
                SASL, NTLM):  # asynchronous SASL and NTLM or reusable strtegy get the bind result synchronously
                    result = response
                else:
                    self.last_error = 'unknown authentication method'
                    if log_enabled(ERROR):
    ...
    def unbind(self,
        ...
        self.last_error = None
        with self.connection_lock:
            if self.lazy and not self._executing_deferred and (
                    self._deferred_bind or self._deferred_open):  # _clear deferred status
                self.strategy.close()
                self._deferred_open = False
                self._deferred_bind = False
                self._deferred_start_tls = False
    ...
    def search(self,
        ...
        """
        conf_attributes_excluded_from_check = [v.lower() for v in
                                               get_config_parameter('ATTRIBUTES_EXCLUDED_FROM_CHECK')]
        if log_enabled(BASIC):
        ...
        with self.connection_lock:
            ...
            if isinstance(paged_size, int):
                if log_enabled(PROTOCOL):
                    log(PROTOCOL, 'performing paged search for %d items with cookie <%s> for <%s>', paged_size,
                        escape_bytes(paged_cookie), self)
    ...
    def compare(self,
        ...
        with self.connection_lock:
            self._fire_deferred()
            request = compare_operation(dn, attribute, value, self.auto_encode,
                                        self.server.schema if self.server else None,
            ...
            if isinstance(response, int):
                ...
                return_value = True if self.result['type'] == 'compareResponse' and self.result[
                    'result'] == RESULT_COMPARE_TRUE else False
                if not return_value and self.result['result'] not in [RESULT_COMPARE_TRUE,
                                                                      RESULT_COMPARE_FALSE] and not self.last_error:
                    self.last_error = self.result['description']
    ...
    def add(self,
        ...
        with self.connection_lock:
            ...
            if _attributes:
                for attr in _attributes:
                    if attr.lower() == 'objectclass':
                        ...
                        attr_object_class = list(_attributes[object_class_attr_name]) if isinstance(
                            _attributes[object_class_attr_name], SEQUENCE_TYPES) else [
                            _attributes[object_class_attr_name]]
                        break
            else:
                _attributes = dict()
            ...
            attr_object_class = [to_unicode(object_class) for object_class in
                                 attr_object_class]  # converts objectclass to unicode in case of bytes value
            _attributes[object_class_attr_name] = reduce(lambda x, y: x + [y] if y not in x else x,
                                                         parm_object_class + attr_object_class,
                                                         [])  # remove duplicate ObjectClasses
            ...
            request = add_operation(dn, _attributes, self.auto_encode, self.server.schema if self.server else None,
                                    validator=self.server.custom_validator if self.server else None,
                                    check_names=self.check_names)
            if log_enabled(PROTOCOL):
            ...
            if isinstance(response, STRING_TYPES + (int,)):
                ...
                return_value = True if self.result['type'] == 'addResponse' and self.result[
                    'result'] == RESULT_SUCCESS else False
                if not return_value and self.result['result'] not in [RESULT_SUCCESS] and not self.last_error:
                    self.last_error = self.result['description']
    ...
    def delete(self,
        ...
        with self.connection_lock:
            ...
            if isinstance(response, STRING_TYPES + (int,)):
                ...
                return_value = True if self.result['type'] == 'delResponse' and self.result[
                    'result'] == RESULT_SUCCESS else False
                if not return_value and self.result['result'] not in [RESULT_SUCCESS] and not self.last_error:
                    self.last_error = self.result['description']
    ...
    def modify(self,
        ...
        with self.connection_lock:
            ...
            for attribute_name in changes:
                ...
                if isinstance(change, SEQUENCE_TYPES) and change[0] in [MODIFY_ADD, MODIFY_DELETE, MODIFY_REPLACE,
                                                                        MODIFY_INCREMENT, 0, 1, 2, 3]:
                    if len(change) != 2:
                    ...
                    for change_operation in change:
                        if len(change_operation) != 2 or change_operation[0] not in [MODIFY_ADD, MODIFY_DELETE,
                                                                                     MODIFY_REPLACE, MODIFY_INCREMENT,
                                                                                     0, 1, 2, 3]:
            ...
            request = modify_operation(dn, changelist, self.auto_encode, self.server.schema if self.server else None,
                                       validator=self.server.custom_validator if self.server else None,
                                       check_names=self.check_names)
            if log_enabled(PROTOCOL):
            ...
            if isinstance(response, STRING_TYPES + (int,)):
                ...
                return_value = True if self.result['type'] == 'modifyResponse' and self.result[
                    'result'] == RESULT_SUCCESS else False
                if not return_value and self.result['result'] not in [RESULT_SUCCESS] and not self.last_error:
                    self.last_error = self.result['description']
    ...
    def modify_dn(self,
        ...
        with self.connection_lock:
            ...
            if isinstance(response, STRING_TYPES + (int,)):
                ...
                return_value = True if self.result['type'] == 'modDNResponse' and self.result[
                    'result'] == RESULT_SUCCESS else False
                if not return_value and self.result['result'] not in [RESULT_SUCCESS] and not self.last_error:
                    self.last_error = self.result['description']
    ...
    def abandon(self,
        ...
        with self.connection_lock:
            ...
            if self.strategy._outstanding or message_id == 0:
                ...
                if (self.strategy._outstanding and message_id in self.strategy._outstanding and
                    self.strategy._outstanding[message_id]['type'] not in ['abandonRequest', 'bindRequest',
                    ...
                    if log_enabled(ERROR):
                        log(ERROR,
                            'cannot abandon a Bind, an Unbind or an Abandon operation or message ID %s not found via <%s>',
                            str(message_id), self)
    ...
    def extended(self,
        ...
        with self.connection_lock:
            ...
            if isinstance(response, int):
                ...
                return_value = True if self.result['type'] == 'extendedResp' and self.result[
                    'result'] == RESULT_SUCCESS else False
                if not return_value and self.result['result'] not in [RESULT_SUCCESS] and not self.last_error:
                    self.last_error = self.result['description']
    ...
    def start_tls(self,
                  read_server_info=True):  # as per RFC4511. Removal of TLS is defined as MAY in RFC4511 so the client can't implement a generic stop_tls method0
        if log_enabled(BASIC):
        ...
        with self.connection_lock:
            ...
            if self.lazy and not self._executing_deferred:
                ...
                if self.closed:
                    self.open()
                if self.server.tls.start_tls(
                        self) and self.strategy.sync:  # for asynchronous connections _start_tls is run by the strategy
                    if read_server_info:
    ...
    def response_to_json(self,
        ...
        with self.connection_lock:
            ...
            if isinstance(search_result, SEQUENCE_TYPES):
                        ...
                        if checked_attributes:
                            if not include_empty:
                                ...
                                entry['attributes'] = dict(
                                    (key, response['attributes'][key]) for key in response['attributes'] if
                        ...
                        if raw:
                            if not include_empty:
                                ...
                                entry['raw_attributes'] = dict(
                                    (key, response['raw_attributes'][key]) for key in response['raw_attributes'] if
                                    response['raw:attributes'][key])
                            else:
                ...
                json_output = json.dumps(json_dict, ensure_ascii=True, sort_keys=sort, indent=indent,
                                         check_circular=True, default=format_json, separators=(',', ': '))
    ...
    def _fire_deferred(self, read_info=None):
        ...
        with self.connection_lock:
            if self.lazy and not self._executing_deferred:
                ...
                if log_enabled(BASIC):
                    log(BASIC, 'executing deferred (open: %s, start_tls: %s, bind: %s) for <%s>', self._deferred_open,
                        self._deferred_start_tls, self._deferred_bind, self)
                try:
                    if self._deferred_open:
                        self.open(read_server_info=False)
                    if self._deferred_start_tls:
                        if not self.start_tls(read_server_info=False):
                            error = 'deferred start_tls not successful' + (
                                ' - ' + self.last_error if self.last_error else '')
                            if log_enabled(ERROR):
                                log(ERROR, '%s for <%s>', error, self)
                            self.unbind()
                    ...
                    if (read_info is None and (not self.server.info and self.server.get_info in [DSA, ALL]) or (
                            not self.server.schema and self.server.get_info in [SCHEMA, ALL])) or read_info:
                        self.refresh_server_info()
        ...
        with self.connection_lock:
            ...
            for attr_set in unique_attr_sets:
                ...
                object_defs.append((attr_set,
                                    object_def,
                                    Reader(self, object_def, search_request['base'], search_request['filter'],
                                           attributes=attr_set) if self.strategy.sync else Reader(self, object_def, '',
                                                                                                  '',
                                                                                                  attributes=attr_set))
