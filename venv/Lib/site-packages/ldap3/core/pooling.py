...
class ServerPoolState(object):
    ...
    def __str__(self):
        ...
        s += ' - Last used server: ' + (
            'None' if self.last_used_server == -1 else str(self.server_states[self.last_used_server].server))
    ...
    def refresh(self):
        ...
        for server in self.server_pool.servers:
            self.server_states.append(
                ServerState(server, datetime(MINYEAR, 1, 1), True))  # server, smallest date ever, supposed available
        ...
        if self.server_states:
            if self.server_pool.strategy == FIRST:
                ...
                if self.server_pool.active:
                    ...
                    self.last_used_server = self.last_used_server + 1 if (self.last_used_server + 1) < len(
                        self.server_states) else 0
    ...
    def find_active_random_server(self):
        ...
        while counter:
            ...
            while temp_list:
                ...
                if not server_state.available:  # server is offline
                    if (isinstance(self.server_pool.exhaust, bool) and self.server_pool.exhaust) or (
                            datetime.now() - server_state.last_checked_time).seconds < self.server_pool.exhaust:  # keeps server offline
                        if log_enabled(NETWORK):
                            log(NETWORK, 'server <%s> excluded from checking because it is offline',
                                server_state.server)
                        continue
                    if log_enabled(NETWORK):
        ...
        raise LDAPServerPoolExhaustedError(
            'no random active server available in server pool after maximum number of tries')
                    ...
                    if (isinstance(self.server_pool.exhaust, bool) and self.server_pool.exhaust) or (
                        ...
                        if log_enabled(NETWORK):
                            if isinstance(self.server_pool.exhaust, bool):
                                log(NETWORK, 'server <%s> excluded from checking because is offline',
                                    server_state.server)
                            else:
                                log(NETWORK, 'server <%s> excluded from checking because is offline for %d seconds',
                                    server_state.server, (self.server_pool.exhaust - (
                                                datetime.now() - server_state.last_checked_time).seconds))
                        continue
                    if log_enabled(NETWORK):
                        log(NETWORK, 'server <%s> reinserted in pool', server_state.server)
                server_state.last_checked_time = datetime.now()
