...
class ObjectDef(object):
    ...
    def __init__(self, object_class=None, schema=None, custom_validator=None, auxiliary_class=None):
        ...
        if isinstance(schema, Connection) and (
                schema._deferred_bind or schema._deferred_open):  # probably a lazy connection, tries to bind
        ...
        if self._schema:
            object_class = [schema.object_classes[name].name[0] for name in
                            object_class]  # uses object class names capitalized as in schema
    ...
    def _populate_attr_defs(self, object_name):
        if object_name in self._schema.object_classes:
            ...
            self.__dict__['_oid_info'].append(
                object_name + " (" + constant_to_class_kind(object_schema.kind) + ") " + str(object_schema.oid))
    ...
    def __repr__(self):
        ...
        r += 'MUST: ' + ', '.join(
            sorted([attr for attr in self._attributes if self._attributes[attr].mandatory])) + linesep
        r += 'MAY : ' + ', '.join(
            sorted([attr for attr in self._attributes if not self._attributes[attr].mandatory])) + linesep
    ...
    if str is not bytes:  # Python 3
        def __bool__(
                self):  # needed to make the objectDef appears as existing in "if cursor:" even if there are no entries
            return True
    else:  # Python 2
        ...
        if isinstance(definition, STRING_TYPES):
            ...
            if definition.key.lower() not in conf_attributes_excluded_from_object_def:
                if definition.key not in self._attributes:
                    ...
                    other_names = [name for name in definition.oid_info.name if
                                   definition.key.lower() != name.lower()] if definition.oid_info else None
                    if other_names:
