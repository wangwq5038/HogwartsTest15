...
from . import STATUS_VIRTUAL, STATUS_WRITABLE, STATUS_PENDING_CHANGES, STATUS_COMMITTED, STATUS_DELETED, \
    STATUS_INIT, STATUS_READY_FOR_DELETION, STATUS_READY_FOR_MOVING, STATUS_READY_FOR_RENAMING, \
    STATUS_MANDATORY_MISSING, STATUSES, INITIAL_STATUSES
...
class EntryState(object):
    ...
    def __repr__(self):
        if self.__dict__ and self.dn is not None:
            r = 'DN: ' + to_stdout_encoding(self.dn) + ' - STATUS: ' + ((
                                                                                    self._initial_status + ', ') if self._initial_status != self.status else '') + self.status + ' - READ TIME: ' + (
                    self.read_time.isoformat() if self.read_time else '<never>') + linesep
            r += 'attributes: ' + ', '.join(sorted(self.attributes.keys())) + linesep
            r += 'object def: ' + (', '.join(
                sorted(self.definition._object_class)) if self.definition._object_class else '<None>') + linesep
    ...
    def set_status(self, status):
        conf_ignored_mandatory_attributes_in_object_def = [v.lower() for v in get_config_parameter(
        ...
        if self.status == STATUS_VIRTUAL or (
                self.status == STATUS_PENDING_CHANGES and self._initial_status == STATUS_VIRTUAL):  # checks if all mandatory attributes are present in new entries
            for attr in self.definition._attributes:
                if self.definition._attributes[
                    attr].mandatory and attr.lower() not in conf_ignored_mandatory_attributes_in_object_def:
...
class EntryBase(object):
    ...
    def __repr__(self):
        if self.__dict__ and self.entry_dn is not None:
            r = 'DN: ' + to_stdout_encoding(self.entry_dn) + ' - STATUS: ' + ((
                                                                                          self._state._initial_status + ', ') if self._state._initial_status != self.entry_status else '') + self.entry_status + ' - READ TIME: ' + (
                    self.entry_read_time.isoformat() if self.entry_read_time else '<never>') + linesep
            if self._state.attributes:
                for attr in sorted(self._state.attributes):
                    if self._state.attributes[attr] or (
                            hasattr(self._state.attributes[attr], 'changes') and self._state.attributes[attr].changes):
    ...
    def entry_mandatory_attributes(self):
        return [attribute for attribute in self.entry_definition._attributes if
                self.entry_definition._attributes[attribute].mandatory]
    ...
    def entry_attributes_as_dict(self):
        return dict((attribute_key, deepcopy(attribute_value.values)) for (attribute_key, attribute_value) in
                    self._state.attributes.items())
    ...
    def entry_to_json(self, raw=False, indent=4, sort=True, stream=None, checked_attributes=True, include_empty=True):
        ...
        if checked_attributes:
            if not include_empty:
                ...
                json_entry['attributes'] = dict(
                    (key, self.entry_attributes_as_dict[key]) for key in self.entry_attributes_as_dict if
                    self.entry_attributes_as_dict[key])
            else:
                json_entry['attributes'] = self.entry_attributes_as_dict
        if raw:
            if not include_empty:
                # needed for python 2.6 compatibility
                json_entry['raw'] = dict((key, self.entry_raw_attributes[key]) for key in self.entry_raw_attributes if
                                         self.entry_raw_attributes[key])
            else:
    ...
    def entry_writable(self, object_def=None, writer_cursor=None, attributes=None, custom_validator=None,
        ...
        if object_def is None:
            if self.entry_cursor.definition._object_class:
                ...
                auxiliary_class = self.entry_definition._auxiliary_class + (
                    auxiliary_class if isinstance(auxiliary_class, SEQUENCE_TYPES) else [])
        ...
        for attr in writable_entry._state.origin.entry_definition._attributes:
            ...
            if attr != original_attr.name and (
                    attr not in writable_entry._state.attributes or conf_operational_attribute_prefix + original_attr.name not in writable_entry._state.attributes):
...
class WritableEntry(EntryBase):
    ...
    def __setattr__(self, item, value):
        conf_attributes_excluded_from_object_def = [v.lower() for v in
                                                    get_config_parameter('ATTRIBUTES_EXCLUDED_FROM_OBJECT_DEF')]
        ...
        if value is not Ellipsis:  # hack for using implicit operators in writable attributes
            ...
            if item in self.entry_cursor.definition._attributes or item.lower() in conf_attributes_excluded_from_object_def:
                if item not in self._state.attributes:  # setting value to an attribute still without values
                    new_attribute = WritableAttribute(self.entry_cursor.definition._attributes[item], self,
                                                      cursor=self.entry_cursor)
                    self._state.attributes[
                        str(item)] = new_attribute  # force item to a string for key in attributes dict
    ...
    def __getattr__(self, item):
        if isinstance(item, STRING_TYPES):
            ...
            if item in self.entry_definition._attributes:  # item is a new attribute to commit, creates the AttrDef and add to the attributes to retrive
                self._state.attributes[item] = WritableAttribute(self.entry_definition._attributes[item], self,
                                                                 self.entry_cursor)
    ...
    def entry_commit_changes(self, refresh=True, controls=None, clear_history=True):
        ...
        if self.entry_status == STATUS_READY_FOR_DELETION:
            ...
            result = self.entry_cursor.connection.modify_dn(self.entry_dn, '+'.join(safe_rdn(self.entry_dn)),
                                                            new_superior=self._state._to)
            if not self.entry_cursor.connection.strategy.sync:
                response, result, request = self.entry_cursor.connection.get_response(result, get_request=True)
            else:
                if self.entry_cursor.connection.strategy.thread_safe:
            ...
            for attr in self.entry_mandatory_attributes:
                if (attr not in self._state.attributes or self._state.attributes[
                    attr].virtual) and attr not in self._changes:
                    missing_attributes.append('\'' + attr + '\'')
            error_message = 'mandatory attributes %s missing in entry %s' % (
            ', '.join(missing_attributes), self.entry_dn)
            if log_enabled(ERROR):
                log(ERROR, '%s for <%s>', error_message, self)
            raise LDAPCursorError(error_message)
            ...
            if self._changes:
                if self.entry_definition._auxiliary_class:  # checks if an attribute is from an auxiliary class and adds it to the objectClass attribute if not present
                    for attr in self._changes:
                        ...
                        attr_classes = self.entry_cursor.schema.attribute_types[attr].mandatory_in + \
                                       self.entry_cursor.schema.attribute_types[attr].optional_in
                            ...
                            if self._state.origin and self.entry_cursor.connection.server == self._state.origin.entry_cursor.connection.server:  # updates original read-only entry if present
                                for attr in self:  # adds AttrDefs from writable entry to origin entry definition if some is missing
                                    if attr.key in self.entry_definition._attributes and attr.key not in self._state.origin.entry_definition._attributes:
                                        self._state.origin.entry_cursor.definition.add_attribute(
                                            self.entry_cursor.definition._attributes[
                                                attr.key])  # adds AttrDef from writable entry to original entry if missing
                                ...
                                for attr in self:  # returns the whole attribute object
                                    if not hasattr(attr, 'virtual'):
                                        self._state.origin.__dict__[attr.key] = self._state.origin._state.attributes[
                                            attr.key]
