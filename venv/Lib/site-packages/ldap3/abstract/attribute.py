...
from . import STATUS_PENDING_CHANGES, STATUS_VIRTUAL, STATUS_READY_FOR_DELETION, STATUS_READY_FOR_MOVING, \
...
class Attribute(object):
    ...
    def __init__(self, attr_def, entry, cursor):
        ...
        self.entry = entry
        self.cursor = cursor
        other_names = [name for name in attr_def.oid_info.name if
                       self.key.lower() != name.lower()] if attr_def.oid_info else None
        self.other_names = set(
            other_names) if other_names else None  # self.other_names is None if there are no short names, else is a set of secondary names
...
class WritableAttribute(Attribute):
    ...
    def _update_changes(self, changes, remove_old=False):
        ...
        if log_enabled(PROTOCOL):
            log(PROTOCOL, 'updated changes <%r> for <%s> attribute in <%s> entry', changes, self.key,
                self.entry.entry_dn)
    ...
    def add(self, values):
        ...
        if values is not None:
            validated = self.definition.validate(
                values)  # returns True, False or a value to substitute to the actual values
