...
class HTTP2StateProtocol:
    ...
    def assemble_response(self, response):
        ...
        return list(itertools.chain(
            self._create_headers(headers, stream_id,
                                 end_stream=(response.content is None or len(response.content) == 0)),
    ...
    def _receive_transmission(self, stream_id=None, include_body=True):
        ...
        while True:
            frm = self.read_frame()
            if (
                    (isinstance(frm, hyperframe.frame.HeadersFrame) or isinstance(frm,
                                                                                  hyperframe.frame.ContinuationFrame)) and
